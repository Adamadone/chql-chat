...
- **Equivalence check.** Same logic in both runners: back-to-back MCP calls for actual + expected CHQL, compare row-set hashes via `hashMCPResponseText` (from chql-core). Only meaningful for `equivalence`-graded questions; the predicate dispatcher skips it for the others.
