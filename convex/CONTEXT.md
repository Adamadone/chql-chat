...
- **Prompt composer (local-only).** In `processMessage`, when `modelId` starts with `local/`, the system prompt is composed via `routeSections(userMessageTexts)` from chql-core, shipping only the conditional CHQL_REFERENCE sections the user actually mentioned. Cloud providers always get `sections: 'all'` to keep the Anthropic prompt-cache prefix stable. Kill-switch: `PROMPT_COMPOSE_LOCAL=off` env var reverts local sessions to the full prompt without redeploy (flip from Convex dashboard).
...
- **`computeEquivalence` shortcuts identical CHQL** with `normalizeChql(actual) === normalizeChql(expected) ⇒ "equivalent"` — same fast-path `rejudgeRun` uses on historical rows, no chy.stat calls. Otherwise it fetches actual + expected back-to-back (~100ms apart) so data drift during a run can't invalidate the hash comparison. The expected leg is cached per run on `evalRuns.expectedHashes` so the same `expectedChql` shared across questions only hits chy.stat once. Equivalence is set-based — empty-vs-empty counts as `"equivalent"` only when both hashes match; otherwise `"expected_empty"` covers the "can't judge" cases.
- **`MCP_CALL_TIMEOUT_MS = 180_000`** passed to every `mcpClient.callTool`. The SDK default is 60s, which fires before chy.stat can finish a slow `pageSize=1000` request; 180s lets the eval (and production) survive a contended chy.stat.
