...
- **Eval scheduler chain.** `runQueryAction` runs one query attempt per scheduled action (own 5-min budget), then schedules the next: retry on failure (attempt 2), advance to next query, or `finalizeEvalRun` when done. Errors are caught and recorded as failure rows — re-throwing would orphan the run.
...
- **`runQueryAction` only marks `success` when the equivalence verdict is `"equivalent"`.** Rate metrics use attempt-1 rows; cost sums every attempt.
- **`finalizeEvalRun` vs `recomputeRunAggregates`.** Both call `computeAggregateMetrics`. `finalizeEvalRun` also sets `status` and `completedAt`; `recomputeRunAggregates` leaves them intact (used by `rejudgeRun`).
