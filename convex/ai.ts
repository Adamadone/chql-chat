...
interface ToolUseMetadata {
  ...
  toolCalls?: string[];
  /** Verification score (0.0–1.0) comparing API response values against the LLM's text. */
  verificationScore?: number;
  /** Breakdown of how many values were checked and how many matched. */
...
/**
 * Result of verifying the LLM's text response against the raw API data.
 *
 * The verification extracts all significant leaf values (numbers and strings)
 * from the API response JSON and checks whether each one appears verbatim
 * in the LLM's rendered text. The score is the ratio of matched values.
 *
 * @see {@link verifyResponse} for the implementation
 */
interface VerificationResult {
  /** Ratio of matched values to total values (0.0 to 1.0). */
  score: number;
  /** Total number of significant leaf values extracted from the API response. */
  totalValues: number;
  /** Number of extracted values found verbatim in the LLM's text. */
...
/**
 * Minimum character length for a leaf value to be considered "significant"
 * and included in verification. Filters out structural noise like single
 * characters, empty strings, and very short tokens that would cause
 * false positives (e.g. a value of "1" appearing coincidentally in the text).
 */
const MIN_SIGNIFICANT_VALUE_LENGTH = 2;

/**
 * Recursively extracts all significant leaf values (strings and numbers)
 * from a parsed JSON object.
 *
 * Walks the entire object tree and collects:
 * - **Numbers** → converted to their string representation (e.g. `80.5` → `"80.5"`)
 * - **Strings** → kept as-is, filtered by minimum length
 * - **Booleans/null** → skipped (not meaningful for measurement verification)
 * - **Arrays/Objects** → recursed into
 *
 * The resulting set contains deduplicated values ready for substring matching
 * against the LLM's text output.
 *
 * @param obj - Any parsed JSON value (object, array, string, number, etc.)
 * @returns A Set of unique string representations of all significant leaf values
 */
function extractLeafValues(obj: unknown): Set<string> {
  ...
  function walk(node: unknown): void {
    ...
    if (typeof node === "number") {
      // Convert number to string exactly as JSON.stringify would (no rounding)
      const str = String(node);
      if (str.length >= MIN_SIGNIFICANT_VALUE_LENGTH) {
        values.add(str);
      }
      return;
    }
    ...
    if (typeof node === "string") {
      if (node.length >= MIN_SIGNIFICANT_VALUE_LENGTH) {
        values.add(node);
      }
      return;
    }
...
/**
 * Verifies the LLM's text response against the raw API response data.
 *
 * This is a **deterministic, post-hoc verification** that checks whether
 * the measurement values returned by the chy.stat API appear verbatim
 * (exact string match) in the LLM's rendered text. No LLM is involved
 * in the verification — it is pure string comparison.
 *
 * ## Algorithm
 *
 * 1. Parse the API response JSON string
 * 2. Recursively extract all significant leaf values (numbers + strings)
 * 3. For each extracted value, check if it appears as a substring in the LLM text
 * 4. Compute `score = matchedValues / totalValues`
 *
 * ## Edge Cases
 *
 * - If the API response cannot be parsed as JSON → returns score 1.0 (nothing to verify)
 * - If there are no significant values to check → returns score 1.0
 * - Numbers are compared as exact strings: `80.5` must appear literally, `80.50` would not match
 *
 * @param apiResponseText - Raw JSON string from the MCP tool result (stored in metadata.apiResponse)
 * @param llmResponseText - The text response generated by Claude (stored in message.content)
 * @returns A {@link VerificationResult} with the score and breakdown
 */
function verifyResponse(
  ...
  let parsed: unknown;
  try {
    parsed = JSON.parse(apiResponseText);
  } catch {
    // If the API response isn't valid JSON, we can't verify — treat as pass
    return { score: 1.0, totalValues: 0, matchedValues: 0 };
  }

  const leafValues = extractLeafValues(parsed);
  ...
  for (const value of leafValues) {
    if (llmResponseText.includes(value)) {
      matched++;
    }
  }

  const total = leafValues.size;
  const score = total > 0 ? matched / total : 1.0;
...
export const processMessage = action({
  ...
  handler: async (ctx, args): Promise<ProcessMessageResult> => {
    ...
    try {
      ...
      // ── Step 7: Verify the LLM response against raw API data ──────
      if (metadata?.apiResponse && finalResponse) {
        ...
        metadata = {
          ...
          verificationDetails: {
            ...
            matchedValues: verification.matchedValues,
          },
        };
        console.log(
          `[Verification] Score: ${verification.score} (${verification.matchedValues}/${verification.totalValues} values matched)`,
        );
      }

      // ── Step 8: Persist the assistant's response ───────────────────
      await ctx.runMutation(api.messages.send, {
        chatId: args.chatId,
